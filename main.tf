...
resource "aws_route_table_association" "sslo-lab-route-table-association-3" {
  subnet_id             = "${aws_subnet.jumpbox-internal-to-bigip-vips.id}"
  route_table_id        = "${aws_route_table.sslo-lab-route-table.id}"
}

resource "aws_route_table_association" "sslo-lab-route-table-association-4" {
  subnet_id             = "${aws_subnet.inspection_out.id}"
  route_table_id        = "${aws_route_table.sslo-lab-route-table.id}"
}

resource "aws_route_table_association" "sslo-lab-route-table-association-5" {
...
resource "aws_network_interface" "sslo-lab-jumpbox-to-mgmt" {
  ...
  subnet_id             = "${aws_subnet.jumpbox-to-mgmt.id}"
  security_groups       = ["${aws_security_group.jumpbox_to_mgmt.id}","${aws_security_group.bigip_to_internal_webserver.id}","${aws_security_group.inspection_zone.id}","${aws_security_group.jumpbox_to_bigip_vips.id}"]
  tags = {
...
#
# Create Inspection Subnet out from BIG-IP to Palo 10.0.5.0/24
#
...
#
# Create Inspection Subnet in from Palo to BIG-IP 10.0.6.0/24
#
...
resource "aws_network_interface" "sslo-lab-firewall-mgmt" {
  ...
  security_groups       = ["${aws_security_group.jumpbox_to_mgmt.id}","${aws_security_group.inspection_zone.id}"]
  tags = {
